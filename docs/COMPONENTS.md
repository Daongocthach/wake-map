...
All 41 public shared components live in `src/common/components/`. Import from the barrel:
...
All components use the **unistyles variants API** for declarative variant/size/state styling. Styles are always in a separate `.styles.ts` file with `variants` blocks and `UnistylesVariants` type export. Interactive components such as `Button`, `IconButton`, `Card`, `Chip`, `ListItem`, and `Menu` use the current project interaction patterns.

Icon components use Lucide icon components, not string icon names. Pass them as `icon={Home}` / `icon={X}` according to the component API.
...
| `labelStyle` | `StyleProp<TextStyle>`                             | `undefined` | Additional label styles                 |
...
| `color`              | `string`                                           | `undefined` | Optional icon color override                  |
...
| `color`    | `TextColor`                                                                           | `theme.colors.text.primary` | Semantic text color token  |
...
Container with themed background, padding, optional press behavior, and customizable left indicator line.
...
| `indicatorColor` | `string`                               | `undefined` | Color of the vertical indicator bar on the left edge |
| `indicatorWidth` | `number`                               | `4`         | Width of the left indicator bar                      |
...
// Card with left indicator (e.g. status)
<Card variant="elevated" indicatorColor={theme.colors.state.error}>
  <Text variant="h3">System Error</Text>
  <Text variant="bodySmall">An unexpected error has occurred.</Text>
</Card>
...
<ListItem
  ...
  subtitle={t('settings.apiUrl.subtitle')}
  left={<Icon name="information-circle-outline" variant="primary" />}
...
| `icon`        | `LucideIcon` | no       | Optional Lucide icon component |
...
| ------------- | ----------------------------------------------------------------------------------------- | ----------- | --------------------------------- |
| `icon`        | `LucideIcon`                                                                              | required    | Lucide icon component             |
| `variant`     | `'primary' \| 'secondary' \| 'tertiary' \| 'muted' \| 'inverse' \| 'accent' \| 'onBrand'` | `'primary'` | Semantic color                    |
| `size`        | `number`                                                                                  | `undefined` | Explicit icon size in pixels      |
| `sizeVariant` | `'sm' \| 'md' \| 'lg'`                                                                    | `'md'`      | Maps to the theme icon size scale |

**Usage**

```typescript
import { Icon } from '@/common/components/Icon';
import { Home, AlertCircle, CheckCircle2, X } from 'lucide-react-native';

<Icon icon={Home} variant="primary" />
<Icon icon={AlertCircle} variant="accent" size={24} />
<Icon icon={CheckCircle2} variant="accent" />
<Icon icon={X} variant="muted" size={20} />
```

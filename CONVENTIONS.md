...
| i18n locale files   | lowercase                      | `en.json`, `vi.json`, `zh-CN.json`, `zh-TW.json` |
| Feature directory   | lowercase, kebab-case          | `src/features/heat-treatment/`                   |
...
- Feature folder names under `src/features/` MUST use `kebab-case` when the name contains multiple words.
- Use lowercase single-word names only when the feature is a single word, such as `auth`, `home`, or `tools`.
...
```
theme.colors.brand.*       - primary, secondary, tertiary, primaryVariant, secondaryVariant, onBrand
theme.colors.background.*  - app, surface, surfaceAlt, section, elevated, input, disabled, modal
theme.colors.text.*        - primary, secondary, tertiary, muted, inverse, accent, link, linkHover, onBrand
theme.colors.border.*      - default, subtle, strong, focus, disabled
theme.colors.icon.*        - primary, secondary, tertiary, muted, inverse, accent, onBrand
theme.colors.state.*       - success, successBg, warning, warningBg, error, errorBg, info, infoBg, disabled
theme.colors.overlay.*     - modal, pressed, hover, focus, ripple, shadow
theme.colors.gradient.*    - primary[], secondary[], accent[], success[], error[], warning[], highlight[]
...
import { createProduct, fetchProduct, fetchProducts } from '../services/productService';
...
All locale files must be updated together:
...
This app currently ships with English, Vietnamese, Simplified Chinese, and Traditional Chinese locales. Do not hardcode layout direction assumptions into components; rely on flexbox and theme tokens.
...
```typescript
// In a service function - unwrap the response and let the caller handle errors
...
The project runs `validate` (type-check + lint + format + i18n) via Husky. All checks must pass before committing.

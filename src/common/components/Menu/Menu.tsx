 ...
 * ```
 */
export function Menu({ visible, onDismiss, anchor, items, align = 'auto' }: MenuProps) {
  ...
  if (anchorLayout) {
    ...
    const resolvedMenuWidth = menuWidth || 280;
    if (align === 'start') {
      menuLeft = anchorLayout.pageX;
    } else if (align === 'end') {
      menuLeft = anchorLayout.pageX + anchorLayout.width - resolvedMenuWidth;
    } else {
      menuLeft = anchorLayout.pageX;
    }

    menuLeft = Math.min(menuLeft, screenWidth - resolvedMenuWidth - 8);
